...
public class ZombieController : MonoBehaviour
{
  ...
  void Update()
  {
    /*if (Input.GetKeyDown(KeyCode.P))
    {
      ...
      if (rd.transform.Find("Zombie:Hips") == true)
      {
        rd.transform.Find("Zombie:Hips").GetComponent<Rigidbody>().AddForce(Camera.main.transform.forward * 10000);
      }
    ...
    switch (state)
    {
      ...
      case STATE.WANDER:
        if (!agent.hasPath)
        {
          float newX = this.transform.position.x + Random.Range(-30, 30);
          float newZ = this.transform.position.z + Random.Range(-30, 30);
